import { find, trim, isNil, mergeRight } from 'ramda';
import {
  readCertificateInfo,
  getPublicKey,
  verifySigningChain
...
export const validateSSLCert = async (cert, options = {}) => {
...
export const validateSSLKey = async (key, options) => {
...
export const validateSSL = async (cert, options) => {
  if (isNil(options)) { options = {}; }
  cert = String(cert);
...
export const validateCertKeyPair = (cert, key, options = {}) => {
  const opts = mergeRight(options, { key: String(key) });
  return validateSSL(cert, opts);
};

export const validateCertToDomain = (cert, domain, options = {}) => {
  const opts = mergeRight(options, { domain: String(domain) });
  return validateSSL(cert, opts);
};

export const validateCertBundle = (cert, certBundle, options = {}) => {
