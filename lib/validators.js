const { find, trim, isNil, merge } = require('ramda');
const {
  readCertificateInfo,
  getModulus,
  verifySigningChain
...
const validateAnyCert = async (cert, options) => {
...
const validateCertKeyPair = (cert, key, options = {}) => {
  const opts = merge(options,  { key: String(key) });
  return validateAnyCert(cert, opts);
};

const validateCertToDomain = (cert, domain, options = {}) => {
  const opts = merge(options,  { domain: String(domain) });
  return validateAnyCert(cert, opts);
};

const validateCertBundle = (cert, certBundle, options = {}) => {
  const opts = merge(options,  { bundle: String(certBundle) });
  return validateAnyCert(cert, opts);
};
